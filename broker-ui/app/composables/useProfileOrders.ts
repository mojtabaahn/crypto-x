...
export const useProfileOrders = () => {
  const fetchOrders = async (filters?: { symbol?: string; side?: string; status?: string }) => {
    ...
    try {
      ...
      const response = await $fetch(`/api/profile/order?${params.toString()}`, {
  ...
  return {
    ...
    profileOrdersLoading: loading,
    profileOrdersError: error,
    fetchProfileOrders: fetchOrders,
    updateProfileOrder: updateOrder,
    addProfileOrder: addOrder
  }
}
